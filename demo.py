"""
초등학생용 영어 Short Story 및 Teacher's Talk Script 자동 생성 에이전트 데모
"""
...
def main():
    ...
    # 예시 Unit 데이터
    example_unit_data = agent.get_example_unit_data()
    
    print("\n📝 입력 데이터:")
    print(f"목표 의사소통 기능: {example_unit_data['target_communicative_functions']}")
    print(f"목표 문법 형태: {example_unit_data['target_grammar_forms']}")
    print(f"목표 어휘: {example_unit_data['target_vocabulary']}")
    
    print("\n🔄 자료 생성 중...")
    
    try:
        # 자료 생성
        result = agent.generate_teaching_materials(example_unit_data)
